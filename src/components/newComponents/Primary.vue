<template>
    <div class="c-author">
        ...
        <div class="m-common-box m-user-bio">
            <div class="u-label"><i class="el-icon-edit"></i><span>{{ $t('TA的签名') }}</span></div>
            <div class="u-userBio">
                {{ data.user_bio || $t('这个人太懒了~没有写签名。') }}
            </div>
        </div>
        <div class="m-common-box m-links" v-if="data != '' && (data.weibo_id || data.github_id || data.tv_id)">
            <div class="u-label"><i class="el-icon-user"></i><span>{{ $t('TA的信息') }}</span></div>
